...
export function apply(ctx, entryConfig) {
  ...
  /**
   * A sub2api gateway's own rate-multiplier discovery endpoint
   * (GET {origin}/v1/sub2api/billing, same API key as /v1/usage):
   * https://github.com/Wei-Shaw/sub2api/issues/4096. Best-effort — a provider
   * pointed at a non-sub2api or older gateway 404s here and the balance
   * snapshot simply carries no rateMultiplier field.
   */
  const querySub2ApiRateMultiplier = async (origin, key) => {
    try {
      const res = await fetch(`${origin}/v1/sub2api/billing`, {
        method: 'GET',
      ...
      const value = Number(raw.effective_rate_multiplier ?? raw.resolved_rate_multiplier)
  ...
  const queryBalance = async (providerName, pcfg) => {
    ...
    try {
      ...
      if (pcfg.parse?.builtin === 'sub2api-usage') {
        const rateMultiplier = await querySub2ApiRateMultiplier(new URL(pcfg.url).origin, key)
        if (rateMultiplier !== undefined) snapshot.rateMultiplier = rateMultiplier
      }
