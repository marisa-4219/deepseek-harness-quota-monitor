 ...
 * and the configuration card on `settings.plugin.item` (the Plugins settings
 * page's `configurable` tab, keyed by this plugin's settings namespace).
 *
 ...
 * needs no import: the provider instance arrives as `ctx.settings`, and its
 * API (`installSection`/`register`/`describe`/`update`/`replace`/`mutate`) is
 * the seam's contract.
...
function ensureHarnessDeps() {
  ...
  const pkg = req('@deepseek-ai/schemastery')
...
export function apply(ctx, entryConfig) {
  ...
  if (!settings || typeof settings.installSection !== 'function') {
    throw new Error('quota-monitor: the settings service (ctx.settings) is unavailable in this composition')
  }
  ...
  // Settings namespace (lowercase hyphenated). This value is also the key the
  // browser card is registered under: `settings.plugin.item` dispatches its
  // cards by the namespace the Host serves, so the client half spells the same
  // literal.
  ...
  // installSection registers the namespace with the composition entry as its
  // base layer, flips the source to the resolved settings scope while a
  // provider is attached, and calls onChange on every committed change.
  settings.installSection(ctx, NS, Config, entryConfig, {
  ...
  const removeProviderFromPatch = (providerName) =>
    removeProviderFromPatchFile(harnessRequire()('js-yaml'), patchFileOf(), providerName)
