...
const BUILTINS = {
  ...
  'commandcode-credits': (raw, opts) => {
    ...
    // Plan table (planId -> monthly credit allowance). The credits response
    // carries no planId, so this normally runs on the id resolved from the
    // provider's sibling subscriptions endpoint (`derived`), or on one the
    // config states outright.
    const PLAN_CREDITS = {
      'individual-go': 10,
      'individual-goat': 70,
      'individual-pro': 30,
...
const PROVIDER_PRESETS = {
  ...
  'commandcode': {
    ...
    currency: 'USD',
    // The descriptive UA below is NOT required: the endpoint answers 200 with a
    // custom UA, with Authorization only, and with an empty UA. It is kept as a
    // courtesy identifier, not because anything rejects its absence.
    headers: { 'user-agent': 'deepseek-harness-quota-monitor' },
    // The credits endpoint reports what REMAINS but never which plan it belongs
    // to, so the monthly allowance is unknowable from that response alone. The
    // plan lives in a sibling endpoint; resolving it here is what lets an
    // AUTO-DISCOVERED provider show the same monthly bar as a hand-configured
    // one, instead of making the user declare a plan id by hand.
    derived: {
      url: 'https://api.commandcode.ai/alpha/billing/subscriptions',
      paths: { planId: ['data.planId', 'planId'] },
      cacheMs: 6 * 3600 * 1000,
    },
  },
  'new-api': {
...
function providerSchema(Schema, WindowSchema) {
  return Schema.object({
    ...
    label: Schema.string(),
    // Optional OVERRIDE for the plan/allowance hint. Command Code's credits
    // endpoint does NOT return a planId, and its monthly window exists only as
    // a derived figure (allowance − remaining). The plugin resolves the plan
    // from the provider's `derived` endpoint on its own, so these are only
    // needed to pin a specific value or to skip that request. Read by parsers
    // through `opts`.
...
export const Config = (() => {
  ...
  return Schema.object({
    ...
    // Spend animation on the sidebar card: a floating "-N tokens" per model call
    // plus a brief shake. Decorative only — the numbers stay on the snapshot
    // refresh cycle, so turning this off changes nothing but motion.
    usageAnimation: Schema.boolean().default(true),
...
export function apply(ctx, config) {
  ...
  // Live subscribers to per-call usage deltas, fed by the llm/stream waterfall
  // below and drained by the SSE endpoint. These drive the card's spend
  // animation only — they never feed the store, which is the single source of
  // truth for every number the card prints.
  const usageListeners = new Set()

  /**
   * Publish one model call's usage to live subscribers.
   *
   * Purely decorative: the animation is a notification about a measurement that
   * has ALREADY been recorded, so a subscriber that is slow, closed, or absent
   * cannot affect accounting. Nothing here may throw into the stream.
   * @param event - provider, model, and the token delta of one call.
   */
  const publishUsage = (event) => {
    if (usageListeners.size === 0) return
    if (cfg().usageAnimation === false) return
    for (const listener of usageListeners) {
      try {
        listener(event)
      } catch { /* a dead subscriber must not disturb the model call */ }
    }
  }
  ...
  /**
   * Resolve parser options that the queried endpoint does not return, from a
   * sibling endpoint the preset names in `derived`.
   *
   * Exists because of Command Code: its credits response reports what REMAINS
   * but not which plan it belongs to, and the plan is what makes a monthly
   * allowance knowable. Requiring the user to hand-write a plan id meant the
   * bar silently disappeared for every AUTO-DISCOVERED provider, which is the
   * default path. Resolving it here gives both paths the same result.
   *
   * A configured value always wins and skips the request entirely, so an
   * explicit `planId`/`monthlyCredits` remains a real override. Anything
   * failing here degrades to `{}` — a missing plan costs the monthly bar, never
   * the balance.
   */
  const derivedCache = new Map() // provider -> { at, value }
  const resolveDerived = async (preset, providerName, pcfg, key) => {
    const spec = preset?.derived
    if (!spec?.url) return {}
    if (pcfg.planId || pcfg.monthlyCredits != null) return {}
    const cached = derivedCache.get(providerName)
    if (cached && Date.now() - cached.at < (spec.cacheMs ?? 6 * 3600 * 1000)) return cached.value
    try {
      ...
      const value = {}
      for (const [optionKey, pathList] of Object.entries(spec.paths ?? {})) {
        for (const path of Array.isArray(pathList) ? pathList : [pathList]) {
          const found = String(path).split('.').reduce((node, part) => (node == null ? node : node[part]), json)
          if (found != null) {
            value[optionKey] = found
            break
          }
        }
      }
      if (Object.keys(value).length > 0) derivedCache.set(providerName, { at: Date.now(), value })
      return value
    } catch {
      return {}
    }
  ...
  const queryBalance = async (providerName, pcfg) => {
    ...
    try {
      ...
      const derived = await resolveDerived(PROVIDER_PRESETS[providerName], providerName, pcfg, key)
      const parsed = parser
        ? await parser(raw, { platform: pcfg.platform, planId: pcfg.planId, monthlyCredits: pcfg.monthlyCredits, ...derived })
        : { kind: 'balance' }
      const snapshot = {
        provider: providerName,
        kind: 'balance',
      ...
      try {
        for await (const chunk of inner) {
          if (chunk && chunk.type === 'usage' && chunk.usage) {
            ...
            usage.record(provider ?? 'unknown', total, {
              input,
              output,
              cacheRead,
              cacheWrite,
            ...
            // Announced after the record lands, so the animation can never
            // describe a measurement the store did not take.
            publishUsage({
              type: 'usage',
  ...
  /**
   * Live usage stream (Server-Sent Events) that drives the card's spend
   * animation.
   *
   * SSE rather than the shipped Remote event bus because
   * `API_REMOTE_FORWARDED_EVENTS` is a FIXED allowlist compiled into
   * `@deepseek-ai/dsh-api-remotes` — a third-party bundle cannot add a key to
   * `ctx.remote.$on`. SSE is a supported transport here: the connection bridge
   * forwards `response.body` chunk by chunk with backpressure, and the web
   * server explicitly exempts `text/event-stream` from gzip (which would
   * otherwise buffer the stream).
   *
   * The stream carries no accounting: every number the card prints still comes
   * from the snapshot route, so a dropped connection costs an animation, never
   * accuracy.
   */
  const streamClosers = new Set()
  ctx.effect(() => () => {
    for (const close of [...streamClosers]) close()
  }, 'quota-monitor: close usage streams on unload')

  ctx.effect(() => connection.fetch.register({
    path: '/api/quota-monitor/events',
    methods: ['GET'],
    requestBody: 'buffered',
    fetch: async (request) => {
      ...
      let heartbeat = null
      let closed = false
      // Hoisted so BOTH teardown paths reach this stream's own cleanup and
      // nothing else's: closing every subscriber from one `cancel()` would take
      // down every other open card.
      const closeSelf = () => {
        if (closed) return
        closed = true
        usageListeners.delete(listener)
        streamClosers.delete(closeSelf)
      ...
      const stream = new ReadableStream({
        start(c) {
          ...
          // The opening frame carries the current toggle, so a card that
          // connects while the animation is off learns that immediately
          // instead of waiting for an event that will never come.
          send({ type: 'hello', animate: cfg().usageAnimation !== false })
          usageListeners.add(listener)
          streamClosers.add(closeSelf)
          // A comment frame every 25s: both proxies and the browser drop an
          // idle event stream, and silence here is normal between model calls.
          heartbeat = setInterval(() => write(': keep-alive\n\n'), 25000)
          request.signal?.addEventListener?.('abort', closeSelf, { once: true })
        },
        cancel() {
          // The consumer went away (navigation, tab close, reload).
          closeSelf()
        },
      })
  ...
  }), 'quota-monitor: usage event stream')
