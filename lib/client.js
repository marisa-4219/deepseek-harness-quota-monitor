 ...
 *   - `settings.plugin.item` (key `quota-monitor`): the provider configuration
 *     card on the Plugins settings page's `configurable` tab. The tab
 *     dispatches cards by settings namespace, so the key is the Host
 *     namespace this plugin registers (`quota-monitor`), and the card is simply
 *     absent from the tab when that namespace is not served.
 *
...
`
		...
		* percentage, or nothing for error/unsupported snapshots. */
		...
		// endpoint of its own.
		let QUOTA_CTX = null
		// Settings namespace literal shared with the host half; also the key the
		// configuration card is registered under on the Plugins page.
		...
		module.exports = {
		  ...
		  // and `slots` owns both UI surfaces. Every face the service proxy hands
		  ...
		  apply(ctx) {
		    ...
		    ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
		      ...
		      QuotaWidget))
		    // Configuration card on the Plugins settings page. The page's
		    // `configurable` tab dispatches every registered card by settings
		    // namespace, so `key` is the namespace the host half registers
		    // (`quota-monitor`): the card appears only while that namespace is
		    // served, and the tab owns the surrounding list/heading chrome.
		    ctx.slots.inject('settings.plugin.item', () => ctx.slots.register(
		      { name: 'settings.plugin.item', key: 'quota-monitor' },
		      QuotaSettings))
		  },
		}
