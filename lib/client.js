...
.qm-rowActions{align-items:center;gap:4px;margin-left:auto;display:inline-flex}
		...
		* percentage, or nothing for error/unsupported snapshots. */
		...
		function QuotaSettings() {
		  ...
		  const rows = providerNames.map((name) => {
		    ...
		    return h('li', { key: name, className: 'qm-rowCard' },
		      h('div', { className: 'qm-rowHead' },
		        ...
		        h('span', { className: 'qm-rowActions' },
		          h('button', { type: 'button', className: 'qm-btnSecondary', disabled: busy, onClick: () => toggleDisabled(name, !disabled) }, disabled ? '启用' : '禁用'),
		  ...
		  const autoRows = autoProviders.length
		    ? h('div', { key: 'auto-group' },
		        ...
		        h('ul', { className: 'qm-rows' },
		          ...autoProviders.map((p) => h('li', { key: p.provider, className: 'qm-rowCard' },
		            h('div', { className: 'qm-rowHead' },
		              h('span', { className: 'qm-rowIdentity' },
		              ...
		              h('span', { className: 'qm-rowActions' },
		                h('button', { type: 'button', className: 'qm-btnSecondary', disabled: busy, onClick: () => toggleDisabled(p.provider, !p.disabled) }, p.disabled ? '启用' : '禁用'),
