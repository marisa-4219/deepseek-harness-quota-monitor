 ...
 *   - `plugins.item` (id `quota-monitor`): the provider configuration page on
 *     the plugin manager's Plugins page. The slot renders each entry twice —
 *     `view: 'summary'` for the card one-liner, `view: 'page'` for the detail
 *     body — so the card must answer both. Registration is gated on
 *     `ctx.configForms.whileServed`, the same pattern the shipped settings
 *     pages use, so a deployment that does not serve the namespace shows no
 *     trace of the page.
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ---------------------------------------------------------------------
		// theme tokens (neutral fallbacks; light/dark safe). Only tokens the
		// shipped Web stylesheet actually defines are used, with a literal
		// fallback so an unthemed host still renders.
		// ---------------------------------------------------------------------
		...
		// The settings card reuses the shipped PluginCard surface parameters
		// (card/editor/field/input/button shapes and tokens); the widget keeps
		// the sidebar card language.
		// ---------------------------------------------------------------------

		const WIDGET_CSS = `
.qm-card{box-sizing:border-box;border-radius:12px;border:.5px solid var(--dsw-alias-border-l4, rgba(128,128,128,.25));background:var(--dsw-alias-bg-layer-3, rgba(128,128,128,.06));padding:10px 12px;display:flex;flex-direction:column;gap:8px;font-family:inherit;font-size:12px;color:var(--dsw-alias-label-primary, inherit);text-align:left;cursor:pointer;width:100%;min-width:0;flex:1;transition:background .15s ease,border-color .15s ease}
.qm-card:hover{background:var(--dsw-alias-bg-layer-4, rgba(128,128,128,.09))}
.qm-card:active{transform:scale(.995)}
.qm-card:focus-visible{outline:2px solid var(--dsw-alias-brand-primary, rgba(128,128,128,.5));outline-offset:2px;border-radius:12px}
.qm-provider{display:flex;flex-direction:column;gap:5px;min-width:0}
.qm-provider + .qm-provider{border-top:.5px solid var(--dsw-alias-border-l2, rgba(128,128,128,.15));padding-top:8px}
...
/* ---- spend animation + refresh state -------------------------------------
   Purely decorative: a "-N" per model call drifting up off the card, and a
   short shake while spend keeps arriving. It never encodes state — the card's
   numbers still arrive on the snapshot refresh cycle. */
/* No overflow:hidden on the card itself: the spend animation escapes the top
   edge by design. The progress track clips its own fill instead. */
.qm-card{position:relative}
.qm-cardBusy{cursor:progress}
/* A 10px dot swapping for a 10px spinner is not feedback: it is a detail the
   eye misses on a 12px card. A refresh also dims the body and runs a bar along
   the top edge, so the state is legible from across the room. */
.qm-cardBusy .qm-body{opacity:.55;transition:opacity .12s ease}
.qm-progress{position:absolute;top:0;left:0;right:0;height:2px;overflow:hidden;background:var(--dsw-alias-interactive-bg-hover, rgba(128,128,128,.15))}
.qm-progressFill{position:absolute;top:0;bottom:0;width:35%;border-radius:2px;background:var(--dsw-alias-state-business-primary, var(--dsw-alias-brand-primary, #3b82f6));animation:qm-slide 1s ease-in-out infinite}
@keyframes qm-slide{0%{left:-35%}100%{left:100%}}
.qm-body{display:flex;flex-direction:column;gap:8px;min-width:0}
.qm-spinner{flex:none;width:10px;height:10px;border-radius:50%;border:1.5px solid var(--dsw-alias-border-l2, rgba(128,128,128,.3));border-top-color:var(--dsw-alias-label-secondary, rgba(128,128,128,.9));animation:qm-spin .7s linear infinite}
@keyframes qm-spin{to{transform:rotate(360deg)}}
.qm-float{position:absolute;right:10px;top:0;pointer-events:none;font-size:11px;line-height:16px;font-weight:600;white-space:nowrap;font-variant-numeric:tabular-nums;color:var(--dsw-alias-state-warn-label, #f59e0b);font-family:var(--ds-font-family-code, ui-monospace,'SF Mono',Consolas,'Courier New',monospace);animation:qm-float 1.15s ease-out forwards}
@keyframes qm-float{0%{opacity:0;transform:translateY(4px)}18%{opacity:1}100%{opacity:0;transform:translateY(-20px)}}
.qm-cardShake{animation:qm-shake .45s cubic-bezier(.36,.07,.19,.97)}
@keyframes qm-shake{10%,90%{transform:translateX(-1px)}20%,80%{transform:translateX(2px)}30%,50%,70%{transform:translateX(-3px)}40%,60%{transform:translateX(3px)}}
@media (prefers-reduced-motion:reduce){
  .qm-cardShake{animation:none}
  .qm-float{animation:qm-float-still 1.15s ease-out forwards}
  .qm-spinner{animation-duration:2s}
}
@keyframes qm-float-still{0%{opacity:0}18%{opacity:1}100%{opacity:0}}
/* ---- settings card: shipped PluginCard visual language ---- */
.qm-section{max-width:720px;color:var(--dsw-alias-label-primary);flex-direction:column;gap:12px;display:flex}
.qm-title{color:var(--dsw-alias-label-primary);margin:0;font-size:15px;font-weight:600;line-height:1.4}
.qm-intro{color:var(--dsw-alias-label-tertiary);margin:0;font-size:13px;line-height:1.5}
.qm-rows{flex-direction:column;gap:8px;margin:12px 0 0;padding:0;list-style:none;display:flex}
.qm-rowCard{border:.5px solid var(--dsw-alias-border-l4);border-radius:16px;flex-direction:column;gap:12px;padding:14px 16px;display:flex;list-style:none}
...
.qm-rowTag{border:.5px solid var(--dsw-alias-border-l4);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;padding:1px 6px;font-size:11px;line-height:16px}
...
.qm-btnPrimary,.qm-btnSecondary,.qm-btnAdd{box-sizing:border-box;height:32px;font:inherit;cursor:pointer;border:1px solid transparent;border-radius:8px;justify-content:center;align-items:center;gap:4px;padding:0 14px;font-size:13px;line-height:1.5;display:inline-flex;font-family:inherit}
.qm-btnPrimary{background:var(--dsw-alias-label-primary);color:var(--dsw-alias-bg-layer-3)}
.qm-btnPrimary:hover:not(:disabled){border-color:var(--dsw-alias-label-primary)}
.qm-btnSecondary,.qm-btnAdd{border-color:var(--dsw-alias-border-l2);color:var(--dsw-alias-label-secondary);background:0 0}
.qm-btnSecondary:hover:not(:disabled),.qm-btnAdd:hover:not(:disabled){color:var(--dsw-alias-label-primary);border-color:var(--dsw-alias-label-dimmed)}
.qm-btnDanger{box-sizing:border-box;height:32px;color:var(--dsw-alias-state-error-primary);font:inherit;cursor:pointer;background:0 0;border:none;border-radius:8px;justify-content:center;align-items:center;padding:0 10px;font-size:13px;line-height:1.5;display:inline-flex;font-family:inherit}
.qm-btnDanger:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-hover-danger)}
.qm-rowActions .qm-btnSecondary,.qm-rowActions .qm-btnDanger{border-radius:8px;height:28px;padding:0 10px;font-size:12px;line-height:18px}
/* enable/disable toggle: borderless on both states; the "disable" action
   carries the danger tint, "enable" stays neutral */
.qm-toggleBtn{box-sizing:border-box;height:28px;border-radius:8px;padding:0 10px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary, rgba(128,128,128,.85));background:0 0;border:none;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-family:inherit}
...
.qm-btnPrimary:focus-visible,.qm-btnSecondary:focus-visible,.qm-btnDanger:focus-visible,.qm-btnAdd:focus-visible{outline:2px solid var(--dsw-alias-brand-primary);outline-offset:-2px}
.qm-editor{background:var(--dsw-alias-bg-layer-2);border-radius:12px;flex-direction:column;gap:14px;padding:14px 16px;display:flex}
...
.qm-input{box-sizing:border-box;border:.5px solid var(--dsw-alias-border-l4);width:100%;height:34px;font:inherit;background:var(--dsw-alias-bg-layer-3);color:var(--dsw-alias-label-primary);border-radius:8px;padding:0 12px;font-size:13px;line-height:1.5;font-family:inherit}
select.qm-input{cursor:pointer;max-width:260px}
.qm-input:focus-visible{border-color:var(--dsw-alias-brand-primary);outline:none}
.qm-input::placeholder{color:var(--dsw-alias-label-dimmed)}
.qm-input:disabled{opacity:.6;cursor:default}
.qm-textarea{min-height:72px;padding:6px 12px;resize:vertical;font-family:var(--ds-font-family-code, ui-monospace,Consolas,monospace)}
.qm-hint{color:var(--dsw-alias-label-tertiary);margin:0;font-size:12px;line-height:1.5}
...
.qm-btnAdd{border:1px dashed var(--dsw-alias-border-l4);border-radius:12px;flex:1 1 0;gap:6px;min-width:180px;height:44px}
.qm-addCard{background:var(--dsw-alias-bg-layer-2);border-radius:12px;flex-direction:column;gap:14px;padding:14px 16px;list-style:none;display:flex}
.qm-addCard .qm-editor{background:0 0;padding:0}
.qm-sectionHeader{align-items:center;gap:10px;display:flex}
.qm-sectionHeader .qm-title{flex:1;min-width:0}
...
/* tokens the shipped stylesheet no longer defines get literal tints: the
   chart only needs three visually distinct hues, not product semantics. */
.qm-tintInput{--meter-tint:var(--dsw-alias-label-primary, #7c93b8)}
.qm-tintOutput{--meter-tint:#a78bfa}
.qm-tintCache{--meter-tint:var(--dsw-static-deepseek-450, #4f8ef7)}
...
/* dark theme: the card sits on the settings surface, so it drops one notch
   deeper (bg-layer-2) than its container and inputs stay on layer 3 */
body[data-ds-dark-theme] .qm-card{background:var(--dsw-alias-bg-layer-2)}
...
`
		...
		* smoothing, gradient strokes and an end dot per line. */
		...
		function Sparkline({ series }) {
		  ...
		  const LINES = [
		    { key: 'i', stroke: 'var(--dsw-alias-label-primary, #7c93b8)', label: '输入' },
		    { key: 'o', stroke: '#a78bfa', label: '输出' },
		    { key: 'c', stroke: 'var(--dsw-static-deepseek-450, #4f8ef7)', label: '缓存' },
		  ]
		...
		* percentage, or nothing for error/unsupported snapshots. */
		function headValue(snap) {
		  ...
		  if (snap.kind === 'windows') {
		    ...
		    // No window caps at all (a balance-only provider on the windows
		    // path): fall back to the balance so the headline is never blank.
		    const b = snap.balance
		    if (b) return `${currencySymbol(b.currency)}${fmtMoney(Number(b.remaining ?? b.total))}`
		    return ''
		  }
		  return ''
		}
		...
		function WindowRow({ w, currency }) {
		  ...
		  // A provider-reported "this window is exhausted" flag: the bar may
		  // still read below 100% (rounding, or a cap raised mid-window), so the
		  // state is worth showing in its own right.
		  if (w.exceeded) title += ' · 已用尽'
		  if (w.percent != null) {
		    const rel = relTime(w.resetsAt)
		    bar = h('div', { className: 'qm-wtrack' },
		      h('div', { className: 'qm-wfill', style: { width: `${Math.min(100, w.percent).toFixed(1)}%`, background: w.exceeded ? TOK.red : percentColor(w.percent) } }))
		    text = `${w.percent}%${rel ? ` · ${rel}` : ''}`
		    if (w.resetsAt) title = `重置于 ${new Date(w.resetsAt).toLocaleString()}${w.exceeded ? ' · 已用尽' : ''}`
		  } else if (w.limitMoney != null && w.usedMoney != null) {
		    const ratio = w.limitMoney > 0 ? w.usedMoney / w.limitMoney : 0
		    bar = h('div', { className: 'qm-wtrack' },
		      h('div', { className: 'qm-wfill', style: { width: `${Math.min(100, ratio * 100).toFixed(1)}%`, background: w.exceeded ? TOK.red : ratioColor(w.usedMoney, w.limitMoney) } }))
		...
		function ProviderBlock({ snap, refreshing }) {
		  // Clicking a provider block must reach the card's refresh handler: the
		  // block fills nearly the whole card, so swallowing the event here left
		  // only the card's own padding clickable and the refresh looked dead.
		  // The one nested interaction that must NOT refresh — the today-used
		  // flip card — stops propagation itself, at the flip face.
		  const head = h('div', { className: 'qm-head' },
		    refreshing
		      ? h('span', { className: 'qm-spinner' })
		      : h('span', { style: { ...S.dot, background: statusColor(snap) } }),
		    h('span', { className: 'qm-name', title: `${snap.label ?? snap.provider}（${snap.provider}）` }, snap.label ?? snap.provider),
		  ...
		  if (snap.error) {
		    const msg = snap.error === 'no-key'
		      ...
		      : snap.error === 'no-url'
		        ? '未配置查询地址'
		        : `查询失败（${snap.error}）`
		    return h('div', { className: 'qm-provider' },
		      head,
		  ...
		  if (snap.kind === 'balance') {
		    ...
		    const used = b && b.used != null
		      ...
		      : null
		    return h('div', { className: 'qm-provider' }, head, used, today)
		  }
		  if (snap.kind === 'windows') {
		    // Some window APIs (Command Code) also report a credit balance;
		    // show it above the bars rather than making the user choose which
		    // half of their quota to look at.
		    const bal = snap.balance
		    const balRow = bal
		      ? h('div', { className: 'qm-sub' },
		          `余额 ${currencySymbol(bal.currency)}${fmtMoney(Number(bal.remaining ?? bal.total))}`,
		          bal.total != null ? ` / ${currencySymbol(bal.currency)}${fmtMoney(Number(bal.total))}` : '',
		          bal.belowThreshold ? ' · 低于阈值' : '')
		      : null
		    return h('div', { className: 'qm-provider' },
		      head,
		      balRow,
		...
		function QuotaWidget(props) {
		  ...
		  const [floats, setFloats] = useState([]) // { id, text } — spend animation
		  ...
		  // Manual refresh is the only caller that shows progress: the 5-minute
		  // interval and post-config-change refreshes must stay silent or the
		  // card would flash on its own. A local round trip can settle in a few
		  // milliseconds, far under a frame, so the indicator is held for a
		  // minimum beat — otherwise the click reads as "nothing happened",
		  // which is exactly the complaint this replaces the toast for.
		  ...
		  // Live spend animation. The stream is decorative: it carries a token
		  // delta per model call and nothing else, so a dropped connection costs
		  // motion, never accuracy — the numbers still come from `load()`.
		  useEffect(() => {
		    if (typeof EventSource !== 'function') return undefined
		    let source
		    try {
		      source = new EventSource('/api/quota-monitor/events')
		    } catch {
		      return undefined
		    }
		    const remove = (id) => setFloats((list) => list.filter((f) => f.id !== id))
		    const onMessage = (event) => {
		      let payload
		      try { payload = JSON.parse(event.data) } catch { return }
		      if (!payload || payload.type !== 'usage') return
		      const tokens = Number(payload.tokens)
		      if (!Number.isFinite(tokens) || tokens <= 0) return
		      const label = payload.provider ?? ''
		      const id = ++floatSeq.current
		      const text = `-${fmt(tokens)}`
		      setFloats((list) => {
		        // Cap concurrent numbers: a burst (parallel subagents) would
		        // otherwise stack unreadable layers on a 12px card.
		        const next = [...list, { id, text, label }]
		        return next.length > 5 ? next.slice(next.length - 5) : next
		      })
		      setTimeout(() => remove(id), 1200)
		      // Restart the shake from zero on every event, so sustained spend
		      // reads as continuous motion rather than one interrupted twitch.
		      const el = cardRef.current
		      if (el) {
		        el.classList.remove('qm-cardShake')
		        void el.offsetWidth // force reflow, else re-adding is a no-op
		        el.classList.add('qm-cardShake')
		      }
		    }
		    const onAnimationEnd = () => cardRef.current?.classList.remove('qm-cardShake')
		    source.addEventListener('message', onMessage)
		    source.addEventListener('error', () => { /* EventSource retries on its own */ })
		    if (cardRef.current) cardRef.current.addEventListener('animationend', onAnimationEnd)
		    return () => {
		  ...
		  const card = (title, ...children) =>
		    h('button', {
		      className: `qm-card${refreshing ? ' qm-cardBusy' : ''}`,
		      type: 'button', title, ref: cardRef,
		      onClick: manualRefresh,
		      disabled: refreshing,
		      onKeyDown: (e) => { if (e.key === 'Enter') manualRefresh() },
		    },
		    // Absolutely positioned, so it sits over the card without joining
		    // the flex column and shifting the content down while refreshing.
		    refreshing ? h('span', { className: 'qm-progress' }, h('span', { className: 'qm-progressFill' })) : null,
		    h('div', { className: 'qm-body' }, ...children),
		    ...floats.map((f) => h('span', { key: f.id, className: 'qm-float', title: f.label }, f.text)))

		  /** Status dot, or the in-flight indicator while a manual refresh runs. */
		  const statusDot = (color) => (refreshing
		    ? h('span', { className: 'qm-spinner' })
		  ...
		  if (!wide) {
		    ...
		    const content = !snap
		      ...
		      : snap.error
		        ? '!'
		        : snap.kind === 'balance'
		          ? `${snap.balance ? `${snap.balance.currency ?? '¥'} ${(snap.balance.remaining ?? snap.balance.total) ?? '—'}` : '—'}`
		          : snap.kind === 'windows'
		            ? (() => {
		                const rows = snap.windows || []
		                const tight = rows
		  ...
		  return card('额度监控（点击刷新）',
		    ...
		    ...(snapshots || []).map((snap) => h(ProviderBlock, { key: snap.provider, snap, refreshing })),
		    (snapshots && snapshots.length === 0 && !error) ? h('div', { className: 'qm-sub' }, '暂无数据') : null)
		}

		// ---------------------------------------------------------------------
		// settings card (Plugins page -> 配置 tab)
		//
		// The Plugins page renders one card per settings namespace it serves and
		// owns the list/heading chrome around it; this component owns everything
		// inside the card. Edits are staged in local state and written on save
		// (the shipped plugin cards behave the same way), with the Host's
		// namespace revision echoed back so a write composed against a stale
		// document is refused rather than silently clobbering a concurrent edit.
		// ---------------------------------------------------------------------

		const EMPTY_PROVIDER = { kind: 'windows', url: '', apiKeyEnv: '', parse: undefined, windows: undefined, lowBalanceThreshold: undefined }
		const BUILTIN_OPTIONS = ['', 'deepseek-balance', 'opencode-go-usage', 'commandcode-credits', 'generic-balance', 'generic-percent-windows', 'new-api-self', 'sub2api-usage', 'sub2api-platform-quotas']
		...
		// Plugin-context faces captured in apply(): the connection handle gives
		// the settings card credentials access through the shipped Remote
		// service (`remote.credentials`), so this plugin ships no credential
		// endpoint of its own.
		let QUOTA_CTX = null
		// Settings namespace literal shared with the host half; also the id the
		// configuration page is registered under on the Plugins page.
		const SETTINGS_NS = 'quota-monitor'
		let widgetRefresh = null // QuotaWidget's load(); QuotaSettings fires it after config changes

		/**
		 * The `plugins.item` slot renders a contribution twice: `view: 'summary'`
		 * for the one-line description on the card, and `view: 'page'` for the
		 * detail body. The shipped pages branch on `props.view`; this one answers
		 * the summary with a short live description and renders the editor for
		 * the page view.
		 */
		function QuotaSettings(props) {
		  if (props && props.view === 'summary') {
		    return '多供应商额度监控：余额查询与本地用量窗口，支持 DeepSeek / OpenCode GO / Command Code 等'
		  }
		  return h(QuotaSettingsPage, props || {})
		}
		...
		function QuotaSettingsPage() {
		  ...
		  // namespace revision the loaded view was composed against; echoed back
		  // on save so a stale write is refused instead of clobbering
		  ...
		  const load = useCallback(async () => {
		    if (!ctx) return
		    try {
		      // Config reads/writes ride the shipped Remote settings transport
		      // (`ctx.remote.settings`), which reads every namespace under
		      // redactSecrets and fences writes by revision. The two host routes
		      // that remain carry what that transport cannot: the provider
		      // catalog (presets) and the profile-patch removal.
		      const described = await ctx.remote.settings.describe()
		      if (!described.ok) throw new Error(described.error.message)
		      const nsView = (described.value.namespaces || []).find((n) => n.ns === SETTINGS_NS)
		      if (!nsView) throw new Error(`设置命名空间 ${SETTINGS_NS} 未注册`)

		      const live = await (await fetch('/api/quota-monitor/settings', { cache: 'no-store' })).json()
		      const value = nsView.value || live.value || {}
		      setView({
		        ...
		        usageAnimation: value.usageAnimation !== false,
		        disabledProviders: value.disabledProviders || {},
		      })
		      setRevision(typeof nsView.revision === 'number' ? nsView.revision : undefined)
		      const layerKeys = (layer) => {
		        const p = layer && layer.providers
		        return p && typeof p === 'object' ? new Set(Object.keys(p)) : new Set()
		      }
		      setBaseKeys(layerKeys(nsView.base))
		      setUserKeys(layerKeys(nsView.user))
		      setAutoProviders(Array.isArray(live.autoProviders) ? live.autoProviders : [])
		      ...
		      if (refs.length) {
		        ...
		        if (credentials.ok) setCreds(credentials.value || {})
		      } else {
		        setCreds({})
		      }
		    } catch (e) {
		  ...
		  const addSelected = () => {
		    ...
		    setView((v) => ({
		      ...
		      providers: {
		        ...v.providers,
		        // Spread the WHOLE preset rather than a hand-picked subset: a
		        // preset may carry headers/auth/platform/windows, and an earlier
		        // subset silently dropped them (Command Code lost the User-Agent
		        // header its endpoint expects). `label`/`note` are presentation
		        // only and are stripped so they never enter the config section.
		        [id]: p
		          ? (({ label, note, ...fields }) => fields)(p)
		   ...
		   *
		   * Every provider lives in at least the user layer, so the write is
		   * always the same revision-fenced `unset` on the shipped settings
		   * transport. A provider the profile patch additionally declares is also
		   * dropped there, because the settings document cannot reach the
		   * composition base layer — that call is idempotent and reports false
		   * when the patch never declared the provider.
		   */
		  const confirmDelete = async () => {
		    const name = deleteTarget
		    if (!name) return
		    const inProfile = baseKeys.has(name)
		    setDeleting(true)
		    setBusy(true)
		    setSaved(false)
		    setFailure(null)
		    try {
		      const res = await ctx.remote.settings.mutate(
		        SETTINGS_NS,
		        [{ op: 'unset', path: ['providers', name] }],
		        revision,
		      )
		      if (!res.ok) {
		        throw new Error(res.error.code === 'settings/conflict'
		          ? '设置已在其他位置更新，请重试'
		          : res.error.message)
		      }
		      if (typeof res.value?.revision === 'number') setRevision(res.value.revision)
		      if (inProfile) {
		        const patchRes = await fetch('/api/quota-monitor/profile-provider', {
		          method: 'POST',
		          headers: { 'content-type': 'application/json' },
		          body: JSON.stringify({ provider: name }),
		        })
		        const patchOut = await patchRes.json().catch(() => ({}))
		        if (!patchRes.ok || patchOut.ok === false) {
		          throw new Error(patchOut.error || `HTTP ${patchRes.status}`)
		        }
		        if (patchOut.removed) setBaseKeys((s) => { const n = new Set(s); n.delete(name); return n })
		        setFailure(patchOut.removed
		          ? '已从 profile 配置与用户配置移除（热重载生效）'
		          : null)
		      }
		      setView((v) => {
		  ...
		  const saveKey = async (name) => {
		    ...
		    setBusy(true)
		    try {
		      // credentials ride the shipped Remote service; the value never
		      // travels back, so the card only learns "configured"
		      const r = await ctx.remote.credentials.set(ref, value)
		  ...
		  const clearKey = async (name) => {
		    ...
		    setBusy(true)
		    try {
		      const r = await ctx.remote.credentials.unset(ref)
		      if (!r.ok) throw new Error(r.error.message)
		  ...
		  const toggleDisabled = async (name, disabled) => {
		    ...
		    try {
		      ...
		      // immediate single-field write: it needs no staging because the
		      // toggle IS the gesture, and the card re-reads the revision it lands on
		      const res = await ctx.remote.settings.mutate(SETTINGS_NS, ops, revision)
		      if (!res.ok) {
		        throw new Error(res.error.code === 'settings/conflict'
		          ? '设置已在其他位置更新，请重试'
		          : res.error.message)
		  ...
		  const save = async () => {
		    ...
		    setFailure(null)
		    try {
		      // Fields this editor owns and normalizes. Anything else the section
		      // carries is passed through untouched: a hand-written whitelist
		      // silently DELETED every field it did not list (a saved `planId`
		      // vanished on the next edit), so the editor must not be the
		      // authority on which config keys exist.
		      const OWNED = new Set(['kind', 'url', 'apiKeyEnv', 'currency', 'auth', 'platform', 'headers', 'parse', 'windows', 'lowBalanceThreshold'])
		      const providers = {}
		      for (const [name, p] of Object.entries(view.providers)) {
		        ...
		        for (const [key, value] of Object.entries(p)) {
		          if (OWNED.has(key) || key in normalized) continue
		          if (value !== undefined && value !== '') normalized[key] = cleanJson(value)
		        }
		        providers[name] = cleanJson(normalized)
		      }
		      const ops = [
		        ...
		        { op: 'set', path: ['usageAnimation'], value: view.usageAnimation !== false },
		        { op: 'set', path: ['disabledProviders'], value: view.disabledProviders || {} },
		      ]
		      // The revision read when this view was composed is echoed back: a
		      // write composed against a document that has since moved is refused
		      // (settings/conflict) instead of silently clobbering the other edit.
		      const res = await ctx.remote.settings.mutate(SETTINGS_NS, ops, revision)
		      if (!res.ok) {
		        throw new Error(res.error.code === 'settings/conflict'
		          ? '设置已在其他位置更新，请放弃修改后重试'
		      ...
		      setSaved(true)
		      if (typeof res.value?.revision === 'number') setRevision(res.value.revision)
		      // re-read: the Host is the only authority on what it accepted
		      await load()
		      if (widgetRefresh) widgetRefresh()
		      return true
		    } catch (e) {
		  ...
		  const globalCard = h('div', { className: 'qm-editor' },
		    h('div', { className: 'qm-editorHeader' },
		      ...
		      h('span', { className: 'qm-editorRoute' }, '阈值 · 刷新 · 自动发现 · 动画')),
		    h('p', { className: 'qm-hint' }, '余额型走供应商查询 API，限额型用本地用量统计；已注册的 LLM 供应商自动进入监控。'),
		    h('label', { className: 'qm-checkRow' },
		      ...
		      '自动监控系统内已注册的 LLM 供应商'),
		    h('label', { className: 'qm-checkRow' },
		      h('input', { type: 'checkbox', checked: view.usageAnimation !== false, onChange: (e) => patchGlobal({ usageAnimation: e.target.checked }) }),
		      '额度消耗时播放动画（卡片飘出 -N 并轻微抖动）'),
		    h('p', { className: 'qm-hint' }, '动画仅为视觉提示，由每次模型调用的用量事件驱动；卡片上的数字仍按上面的刷新间隔更新，关闭动画不影响统计与额度准确性。'),
		    h('div', { className: 'qm-row' },
		  ...
		  // No section heading of its own: the Plugins page wraps this card with
		  // the plugin's name and description, and the tab supplies the page
		  // heading — repeating them here would read as a nested page.
		  return h('div', { className: 'qm-section' },
		...
		module.exports = {
		  // `remote` carries the shipped Remote transport (`remote.settings`,
		  // `remote.credentials`); `connection` provides that transport's stream,
		  // `configForms` reports which settings namespaces the Host serves, and
		  // `slots` owns both UI surfaces. Every face the service proxy hands
		  // out must be named here — a nested namespace does not imply its root.
		  inject: ['slots', 'connection', 'remote', 'remote.settings', 'remote.credentials', 'configForms'],
		  apply(ctx) {
		    QUOTA_CTX = ctx
		    ensureWidgetCss()
		    ctx.slots.inject('sidebar.footer.action', () => ctx.slots.register(
		      ...
		      QuotaWidget))
		    // Provider configuration page on the Plugins page. `whileServed`
		    // keeps the registration alive only while the Host actually serves
		    // the `quota-monitor` namespace, so an unloaded or broken host half
		    // leaves no orphan page behind.
		    ctx.effect(() => ctx.configForms.whileServed([SETTINGS_NS], () => ctx.slots.inject('plugins.item', () => ctx.slots.register(
		      { name: 'plugins.item', id: 'quota-monitor', order: 30, label: '额度监控' },
		      QuotaSettings))), 'quota-monitor: settings page')
		  },
		}
